...
namespace EzySlice {
    ...
    public static class SlicerExtensions {
        ...
        public static SlicedHull Slice(this Mesh sharedMesh, Matrix4x4 worldToLocalMatrix, Vector3 position, Vector3 direction)
            => Slice(sharedMesh, worldToLocalMatrix, position, direction, new TextureRegion(0.0f, 0.0f, 1.0f, 1.0f));

        public static SlicedHull Slice(this Mesh sharedMesh, Matrix4x4 worldToLocalMatrix, Vector3 position, Vector3 direction, TextureRegion textureRegion)
        {
            Plane cuttingPlane = new Plane();
            ...
            Vector3 refUp = inv.MultiplyVector(direction).normalized;
            Vector3 refPt = worldToLocalMatrix.MultiplyPoint(position);

            cuttingPlane.Compute(refPt, refUp);

            return Slicer.Slice(sharedMesh, cuttingPlane, textureRegion, 0);
        }
        ...
        public static bool Slice(this Mesh sharedMesh, Matrix4x4 worldToLocalMatrix, Vector3 position, Vector3 direction, out Mesh lowerMesh, out Mesh upperMesh)
            => Slice(sharedMesh, worldToLocalMatrix, position, direction, new TextureRegion(0.0f, 0.0f, 1.0f, 1.0f), out lowerMesh, out upperMesh);

        public static bool Slice(this Mesh sharedMesh, Matrix4x4 worldToLocalMatrix, Vector3 position, Vector3 direction, TextureRegion textureRegion, out Mesh lowerMesh, out Mesh upperMesh)
        {
            SlicedHull slice = Slice(sharedMesh, worldToLocalMatrix, position, direction, textureRegion);

            if (slice != null)
            {
                lowerMesh = slice.lowerHull;
                upperMesh = slice.upperHull;
                return true;
            }

            lowerMesh = upperMesh = null;
            return false;
        }
