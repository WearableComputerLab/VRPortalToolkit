...
namespace Misc.EditorHelpers
{
    ...
    public static class SortedEditor
    {
        ...
        private static Dictionary<Key, PropertyDrawer> nextInQueue;
        private static Dictionary<Type, Type> drawerTypesByType;
        private static Dictionary<Type, PropertyDrawer> drawersByDrawerType;
        ...
        private static void CachePropertyDrawer(FieldInfo fieldInfo)
        {
            ...
            if (fieldInfo != null && !nextInQueue.ContainsKey(new Key(fieldInfo)))
            {
                ...
                propertyDrawers.Sort((i, j) => i.Key.order.CompareTo(j.Key.order));
        ...
        private static bool TryGetPropertyDrawer(FieldInfo fieldInfo, PropertyAttribute propertyAttribute, out PropertyDrawer propertyDrawer)
        {
            ...
            while (type != null)
            {
                if (drawerTypesByType.TryGetValue(type, out Type drawerType))
                {
                    //if (drawersByDrawerType == null) drawersByDrawerType = new Dictionary<Type, PropertyDrawer>();

                    //if (!drawersByDrawerType.TryGetValue(drawerType, out propertyDrawer))
                    //{
                        try
                        {
                        ...
                        return false;
                        //drawersByDrawerType[drawerType] = propertyDrawer;
                    //}
        ...
        private static void CacheAllDrawers()
        {
            ...
            foreach (Assembly assembly in allAssembliesInDomain)
            {
                ...
                {
                    if (propertyDrawerType.IsAssignableFrom(type) && !type.IsInterface && !type.IsAbstract)
                    {
                        IList<CustomAttributeData> customAttributes = CustomAttributeData.GetCustomAttributes(type);

                        if (customAttributes == null || customAttributes.Count == 0) continue;

                        CustomAttributeData drawerAttribute = customAttributes[0];

                        if (drawerAttribute == null) continue;

                        if (drawerAttribute.ConstructorArguments == null || drawerAttribute.ConstructorArguments.Count == 0) continue;

                        CustomAttributeTypedArgument drawerTypedArgument = drawerAttribute.ConstructorArguments[0];

                        if (drawerTypedArgument == null) continue;

                        Type associatedType = drawerTypedArgument.Value as Type;

                        if (associatedType == null) continue;

                        if (drawerTypesByType.ContainsKey(associatedType)) continue;

                        drawerTypesByType.Add(associatedType, type);
                    }
                }
            }
        }
